...
const MenuItem = ({ item }) => {
  ...
  const SingleLevel = ({ item }) => {
    return (
      <ListItem button  component={Link} to={item.to} style={
        item.type ==='group'  ? {
        ...
        <ListItemText primary={item.title}  primaryTypographyProps={
            item.type ==='group'  ? {
                fontSize:'15x',fontFamily:'Segoe UI'
            } : {
                fontSize:'13px'
            }
            } />
...
export default function MenuProvider({handleDrawer}) {
  ...
  >
    <DrawerHeader>
        <img src='http://103.108.220.162:3050/whatsapp-image-20221222-at-923-3@2x.png' style={{width:'80%',objectFit:'contain'}}></img>
